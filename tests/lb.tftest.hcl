...
run "lb_allow_9200" {
  assert {
    condition = aws_security_group_rule.boundary_lb_allow_9200.from_port == 9200 && aws_security_group_rule.boundary_lb_allow_9200.to_port == 9200
...
run "lb_public_subnets" {
  assert {
    condition = length(setintersection(aws_lb.boundary_lb.subnets, var.public_subnet_ids)) == length(var.public_subnet_ids)

    error_message = "Expected the load balancer to be in the public subnets"
  }
}
